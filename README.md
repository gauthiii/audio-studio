...
The frontend dev server proxies `/api/*` to the backend on port 8000, so no CORS or env config is needed for local development.
...
| Mode | Command | Notes |
|---|---|---|
| Real AI (default) | `uvicorn app.main:app --port 8000` | Uses Meta's `htdemucs`. First run downloads ~1.7GB of weights. CPU works; GPU is much faster. |
| Mock / demo | `STEM_MODE=mock uvicorn app.main:app --port 8000` | Instant frequency-band stems — perfect for testing the mixer UI without the heavy model. |
